...
export const verifyEvents =
  (debug: boolean) =>
  async function* (source: AsyncIterable<BaseEvent>): AsyncIterable<BaseEvent> {
    ...
    for await (const event of source) {
      ...
      if (!firstEventReceived) {
        ...
        if (eventType !== EventType.RUN_STARTED && eventType !== EventType.RUN_ERROR) {
          throw new AGUIError(`First event must be 'RUN_STARTED'`);
        }
      ...
      switch (eventType) {
        ...
        case EventType.STEP_STARTED: {
          ...
          if (activeSteps.has(stepName)) {
            throw new AGUIError(`Step "${stepName}" is already active for 'STEP_STARTED'`);
          }
